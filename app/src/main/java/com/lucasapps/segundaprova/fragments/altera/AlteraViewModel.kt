...
@HiltViewModel
class AlteraViewModel @Inject constructor(private var repository: FilmeRepository) : ViewModel() {
    ...
    fun alterarButtonEvent(p:Filmes){
        filmes.value = p
        viewModelScope.launch {
            withContext(Dispatchers.IO){
                filmes.value?.let { repository.update(p) }
            }
        }
    }
}